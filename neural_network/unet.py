...
class Layer(nn.Module):
    ...
    def __init__(self, input_size, output_size, batch_norm=True, kernel_size=3, padding=1):
    ...
    def forward(self, input_d):
        input_d = self.conv1(input_d)
        if self.batch_norm:
            input_d = self.batch1(input_d)
        input_d = nn.ReLU()(input_d)

        output = self.conv2(input_d)
        if self.batch_norm:
...
class Unet(nn.Module):
    def __init__(self, input_size=1, output_size=1, first_filter_size=64, kernel_size=3, padding=1):
        ...
        self.conv5 = Layer(first_filter_size * 2 ** 3 + first_filter_size * 2 ** 2, first_filter_size * 2 ** 2)
        self.conv6 = Layer(first_filter_size * 2 ** 2 + first_filter_size * 2 ** 1, first_filter_size * 2 ** 1)
        self.conv7 = Layer(first_filter_size * 2 ** 1 + first_filter_size * 2 ** 0, first_filter_size * 2 ** 0)
        self.conv8 = Layer(first_filter_size * 2 ** 0, output_size)
    ...
    def forward(self, input_data):
        ...
        output_data = F.interpolate(output_data6, scale_factor=2)
